...
export const Header: FC<HeaderProps> = ({ categories, config, stores, currentStoreCode }) => (
  <header class="sticky top-0 z-50 bg-base-100/95 backdrop-blur-md border-b border-base-300/50">
    <div class="flex items-center gap-6 max-w-[var(--content-max)] mx-auto px-[var(--content-padding)] h-[var(--header-height)]">
      ...
      <button
        class="hidden max-lg:flex items-center justify-center w-10 h-10 shrink-0 text-base-content rounded-md hover:bg-base-200 transition-colors"
      ...
      {/* Logo — centred on mobile via flex-1 */}
      <a href="/" class="flex items-center shrink-0 max-lg:flex-1 max-lg:justify-center no-underline text-base-content hover:text-base-content" data-turbo-prefetch="true">
        {config.logoUrl ? (
                  ...
                  {cat.children.filter(c => c.includeInMenu).map((child) => (
                    <li key={child.id}>
                      <a
                        ...
                        data-turbo-prefetch="true"
                        class="block px-3 py-1.5 text-sm text-base-content/70 rounded-md no-underline transition-colors hover:text-base-content hover:bg-base-200"
                      >
                        {child.name}
                      </a>
                    </li>
      ...
      <div class="flex items-center gap-1 shrink-0 ml-auto">
        ...
        {/* Search */}
        <button class="flex items-center justify-center w-10 h-10 text-base-content/80 rounded-md hover:text-base-content hover:bg-base-200 transition-colors" onclick="document.dispatchEvent(new CustomEvent('search:open'))" aria-label="Search">
        ...
        <div class="max-lg:hidden" data-controller="auth-state">
          {/* Guest state */}
          <a href="/login" class="flex items-center justify-center w-10 h-10 text-base-content/80 rounded-md hover:text-base-content hover:bg-base-200 transition-colors" data-auth-state-target="guestLink" aria-label="Sign in">
            <svg class="w-5 h-5 shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
          ...
          <div class="dropdown dropdown-end" data-auth-state-target="userMenu" style="display:none">
            <button class="flex items-center justify-center w-10 h-10 text-base-content/80 rounded-md hover:text-base-content hover:bg-base-200 transition-colors" data-action="auth-state#toggleDropdown" aria-label="Account menu">
              <svg class="w-5 h-5 shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/>
              </svg>
            </button>
            <div class="dropdown-content bg-base-100 shadow-lg rounded-lg border border-base-300 py-1 px-1 w-48 z-50" data-auth-state-target="dropdown" style="display:none">
              ...
              <a href="/account" class="block px-3 py-1.5 text-sm text-base-content/70 rounded-md no-underline hover:bg-base-200 hover:text-base-content transition-colors" data-turbo-prefetch="true">My Account</a>
              <a href="/account?tab=wishlist" class="block px-3 py-1.5 text-sm text-base-content/70 rounded-md no-underline hover:bg-base-200 hover:text-base-content transition-colors" data-turbo-prefetch="true">My Wishlist</a>
              <a href="/account?tab=orders" class="block px-3 py-1.5 text-sm text-base-content/70 rounded-md no-underline hover:bg-base-200 hover:text-base-content transition-colors" data-turbo-prefetch="true">My Orders</a>
        ...
        {/* Cart */}
        <a href="/cart" class="flex items-center justify-center w-10 h-10 relative text-base-content/80 rounded-md hover:text-base-content hover:bg-base-200 transition-colors" data-turbo-prefetch="true" data-controller="cart" data-cart-target="badge" aria-label="Shopping cart">
          <svg class="w-5 h-5 shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
