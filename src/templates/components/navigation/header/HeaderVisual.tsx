...
/**
 * Visual Header — megamenu with category image cards.
 * Hovering a top-level nav item opens a panel with subcategories displayed
 * as image cards with label overlays. Great for lifestyle/visual brands.
 */
export const Header: FC<HeaderProps> = ({ categories, config, stores, currentStoreCode }) => {
  const navCats = categories.filter(c => c.level === 2 && c.includeInMenu);
                              ...
                              {child.image ? (
                                <Fragment>
                                  <img
                                    ...
                                    loading="lazy"
                                  />
                                  {/* Gradient label overlay — only with images */}
                                  <div class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/65 to-transparent p-2.5 pt-6">
                                    <span class="text-white text-xs font-semibold drop-shadow-sm">{child.name}</span>
                                  </div>
                                </Fragment>
                              ) : (
                                <div class="w-full h-full flex flex-col items-center justify-center gap-1.5 px-2 group-hover/card:bg-base-200 transition-colors">
                                  <span class="text-sm font-semibold text-base-content/70 text-center leading-tight">{child.name}</span>
                                  <span class="text-[10px] text-base-content/40 font-medium">Shop now</span>
                                </div>
                              )}
                            </a>
                          ))}
