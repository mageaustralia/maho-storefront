 ...
 * Centered Footer
 *
 * DaisyUI footer-center: logo, tagline, social icons, copyright.
 * Clean, minimal, brand-focused. No link columns.
 */
export const FooterCentered: FC<FooterProps> = ({ config, footerPages }) => (
    ...
    <div class="footer footer-center p-10">
      ...
      <aside>
        {config.logoUrl ? (
          ...
          <span class="text-2xl font-extrabold tracking-tight lowercase">{config.storeName}</span>
        )}
        <p class="mt-2 text-neutral-content/60">Providing quality products since 2024</p>
      </aside>

      {/* Navigation links — horizontal */}
      <nav>
        <div class="grid grid-flow-col gap-4">
          <a href="/about-maho-demo-store" class="link link-hover">About</a>
          <a href="/contacts" class="link link-hover">Contact</a>
          <a href="/blog" class="link link-hover">Blog</a>
          <a href="/privacy-policy-cookie-restriction-mode" class="link link-hover">Privacy</a>
          {footerPages
            .filter(p => !['about-maho-demo-store', 'privacy-policy-cookie-restriction-mode', 'contacts'].includes(p.identifier))
            .slice(0, 3)
            .map(p => (
              <a key={p.identifier} href={`/${p.identifier}`} class="link link-hover">{p.title}</a>
            ))
          }
        </div>
      </nav>
