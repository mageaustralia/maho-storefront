    ...
    <article class="card bg-base-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col h-full" data-product-id={String(product.id)} data-product-sku={product.sku}>
      ...
      <div class="px-3 pb-3 mt-auto">
        {isOutOfStock ? (
          ...
          // B2B Access blocks purchasing for this caller. The backend rejects
          // the add outright (422), so offering the button would only produce
          // an error — send them to the PDP, which carries the login prompt.
