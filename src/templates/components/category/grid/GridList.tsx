...
import { CardHorizontal } from '../../product-display/card/CardHorizontal';

import type { CategoryGridProps } from './GridStandard';

/**
 * List-view product grid — single column with horizontal cards.
 */
export const GridList: FC<CategoryGridProps> = ({ products, currency, currentPage, totalPages }) => (
  <>
    <div class="flex flex-col gap-3" data-category-filter-target="grid">
      {products.map((product, i) => (
        <CardHorizontal key={product.id} product={product} currency={currency} priority={i < 4} />
      ))}
    </div>
