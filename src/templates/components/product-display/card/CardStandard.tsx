...
export const ProductCard: FC<ProductCardProps> = ({ product, currency = 'USD', priority = false }) => {
  ...
  // Hover-swatch controller attrs for configurable products
  ...
  return (
    <article
      class="card bg-base-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden flex flex-col h-full relative"
    ...
    >
      ...
      <div class="px-3 pb-3 mt-auto">
        {isOutOfStock ? (
          ...
          <button class="btn btn-sm btn-primary w-full" onclick={`(async()=>{const c=localStorage.getItem('maho_cart_id');if(!c){const r=await fetch(window.MAHO_API_URL+'/api/guest-carts',{method:'POST',headers:{'Accept':'application/ld+json'}});const d=await r.json();localStorage.setItem('maho_cart_id',d.maskedId)}const m=localStorage.getItem('maho_cart_id');const r=await fetch(window.MAHO_API_URL+'/api/guest-carts/'+m+'/items',{method:'POST',headers:{'Accept':'application/ld+json','Content-Type':'application/ld+json'},body:JSON.stringify({sku:'${product.sku}',qty:1})});if(r.ok){document.dispatchEvent(new CustomEvent('cart:updated'));document.dispatchEvent(new CustomEvent('cart:open'));this.innerHTML='Added!';setTimeout(()=>this.innerHTML='<span class=\\'hidden sm:inline\\'>Add to </span>Cart',2000)}else{this.innerHTML='Error';setTimeout(()=>this.innerHTML='<span class=\\'hidden sm:inline\\'>Add to </span>Cart',2000)}})()`}><span class="hidden sm:inline">Add to </span>Cart</button>
        )}
      </div>
      ...
      {isConfigurable && !isOutOfStock && (
        <div
          ...
          class="absolute inset-x-0 bottom-0 bg-base-100/95 backdrop-blur-sm flex flex-col gap-2 p-3 transition-all duration-250 ease-out hidden z-10 border-t border-base-200 rounded-b-[var(--rounded-box)]"
