...
(function () {
  ...
  StripeAdapter.prototype.init = async function (container, context) {
    ...
    try {
      ...
      // Note: Link Authentication Element handles email/identity only.
      // Payment still requires card entry or Google Pay/Apple Pay.
      container.innerHTML =
  ...
  StripeAdapter.prototype.tokenize = async function () {
    ...
    // Link flow — Link authenticated but we still need card details.
    // Link only verifies identity (email + SMS), the actual payment still
    // goes through the card element. Fall through to the card flow below.
    // If Link provided a saved payment method, it would have been handled
    // by the Payment Request Button flow above (via _prPaymentIntentId).
    ...
    // Read shipping method + address from the checkout form so the Worker
    // can set shipping on the cart before creating the PaymentIntent.
    var piBody = { cartId: cartId };
    var shippingRadio = document.querySelector('input[name="shipping_method"]:checked');
    if (shippingRadio) {
      piBody.shippingMethod = shippingRadio.value;
      // Collect address from checkout form targets
      var getVal = function(id) { var el = document.getElementById(id); return el ? el.value.trim() : ''; };
      piBody.shippingAddress = {
        firstName: getVal('checkout-firstname'),
        lastName: getVal('checkout-lastname'),
        street: [getVal('checkout-street'), getVal('checkout-street2')].filter(Boolean),
        city: getVal('checkout-city'),
        postcode: getVal('checkout-postcode'),
        countryId: getVal('checkout-country'),
        regionId: getVal('checkout-region') || undefined,
        region: getVal('checkout-region-text') || undefined,
        telephone: getVal('checkout-telephone'),
      };
    }

    var piResponse = await this._apiPost('/api/payments/stripe/payment-intents', piBody);
