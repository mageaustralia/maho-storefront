...
Target: `<staging-host>` (branch: `feature/admin-theme`)
Files live on staging at `/var/www/<staging-host>/web/` — deployed via SSH, not git-committed yet.
...
```bash
# CSS only
scp -P 22582 sales_orders_files/order-fresh.css <ssh-user>@<staging-host>:/var/www/<staging-host>/web/public/skin/adminhtml/default/default/order-fresh.css

# Flush cache after layout XML changes
ssh <ssh-user>@<staging-host> -p <port> "cd /var/www/<staging-host>/web && php maho cache:flush"
```
